        ...
        <div className='flex items-center justify-between'>
          {steps.map((step, index) => (
            <div key={step.id} className="flex items-center">
              <div className={
                cn('w-10 h-10 rounded-full flex items-center justify-center transition-colors',
                  currentStep === step.id ? 'bg-blue-600 text-white' : currentStep > step.id ? 'bg-green-600 text white' : 'bg-slate-600 text-slate-300'
                )
              }>
              ...
              </div>
              {
                index < steps.length - 1 && (
                  <div
                    className={`w-40 h-px mx-6 transition-colors ${currentStep > step.id ? "bg-green-600" : "bg-slate-600"
                      }`}
                  />
                )
              }
